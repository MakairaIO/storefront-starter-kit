...
function getEndpoint({ isSearch, isSnippet, isRecommend }) {
  ...
  if (isRecommend) return url + '/recommendation/public'
...
export default async function fetchFromMakaira({
  ...
  const url = getEndpoint({ isSnippet, isSearch, isRecommend })
